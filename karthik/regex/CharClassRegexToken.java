...
import karthik.regex.TokenizerException;
...
class CharClassRegexToken extends RegexToken {
    ...
    CharClassRegexToken(char range_start, char range_end, boolean negative,
        int flags) {
        this(negative, flags);
    ...
    private boolean contains(char c) {
        char case_switched_c = Character.isUpperCase(c) ? Character.toLowerCase(
        ...
        if (case_insensitive && (charClassStrings.indexOf(String.valueOf(
            case_switched_c)) != -1))
            return true;
        ...
        for (char[] range : rangeList) {
            ...
            if (case_insensitive)
                if ((Character.compare(range[0], case_switched_c) <= 0)
                    ...
                    return true; // true if case_insensitive matching and c is within the range with its case switched                
        }
        return false;
    }
