...
class TransitionTable implements Cloneable {
    ...
    static TransitionTable get_expanded_backref_table(String match_string,
        List<Integer> groupIDList, TransitionTable parent_table,
        int backref_token_col) {

        return new TransitionTable(match_string, groupIDList, parent_table,
            backref_token_col);
    }

    private TransitionTable(String match_string, List<Integer> groupIDList,
        TransitionTable parent_table,
        int backref_token_col) {
    ...
    TransitionTable get_table_with_backref_expansion_removed(
        ...
        Map<Integer, Matchable> m;
        Matchable tok;
        List<Map<Integer, Matchable>> temp_list = new ArrayList<>(
            len - backref_states);
    ...
    TransitionTable concat(final TransitionTable n2) {
        ...
        for (int i = 0; i < n2Start; i++)
            for (int j = 0; j < n2Start; j++) {
                ...
                if (token != null)
                    setTransition(starting_num_states + i,
                        starting_num_states + j, token);
            }
        ...
        for (int i = n2Start + 1; i < n2States; i++)
            for (int j = 0; j < n2Start; j++) {
                token = n2.getTransition(i, j);
                if (token != null)
                    setTransition(starting_num_states - 1 + i,
                        starting_num_states + j, token);
            }
    ...
    TransitionTable plus(Integer quantGroupID, Integer uniqueID) {
        ...
        expand_table(1);
        setTransition(finish, getNumStates() - 1, new QuantifierToken(tok_type,
            quantGroupID, true, uniqueID));
        finish = getNumStates() - 1;
        ...
        temp.setTransition(oldStart, temp.finish, new QuantifierToken(tok_type,
            quantGroupID, false, uniqueID));
        temp.setTransition(oldFinish, temp.finish, new QuantifierToken(tok_type,
            quantGroupID, false, uniqueID));
        temp.setTransition(oldFinish, oldStart, new QuantifierToken(tok_type,
            quantGroupID, true, uniqueID));
    ...
    TransitionTable star(Integer quantGroupID, Integer uniqueID) {
        ...
        setTransition(start, finish, new QuantifierToken(tok_type, quantGroupID,
            false, uniqueID));
        setTransition(oldFinish, finish, new QuantifierToken(tok_type,
            quantGroupID, false, uniqueID));
        setTransition(oldFinish, oldStart, new QuantifierToken(tok_type,
            quantGroupID, true, uniqueID));
    ...
    private void make_lazy(Integer quantGroupID, Integer uniqueID,
        RegexTokenNames tok_type) {
        ...
        setTransition(start, oldStart, new QuantifierToken(tok_type,
            quantGroupID, true, uniqueID));
    }
    ...
    TransitionTable brace(final int min, final int max, Integer quantGroupID,
        ...
        setTransition(finish, brace_finish, new QuantifierToken(tok_type,
            quantGroupID, false, uniqueID));
        ...
        for (int r = min + 1; r <= max; r++) {
            ...
            concat(temp);
            setTransition(getFinish(), brace_finish, new QuantifierToken(
                tok_type, quantGroupID, false, uniqueID));
        }
        finish = brace_finish;
        return this;
    }
        ...
        for (Map<Integer, Matchable> transMatrixRows : trans_table_list) {
            ...
            for (int i : transMatrixRows.keySet()) {
                ...
                if (tok == null) // should not get to this branch since map will only contain valid tokens
                {
                    sb.append(i + ":");
                    sb.append(
                        transMatrixRows.containsKey(i) ? "contains null; " : "empty; ");
                } else
