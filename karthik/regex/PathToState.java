...
class PathToState {
    ...
    /**
     * ****************************
     * This object stores the path taken to each state to enable submatch
     * recovery Each object contains the string matched so far and the groups to
     * which each character of that string belongs.
     *
     ******************************
     */
    ...
    void processQuantifier(final Integer index, QuantifierToken quant_token) {
        ...
        if (quant_token.isQuantStop()) {
            ...
            if (quant_token.is_lazy() && (!lazy_quantifier_flags.contains(
                uniqueID)))
    ...
    PathToState compare_finish_states(PathToState new_finish) {
        ...
        for (Integer lazy_flag_ID = 0; lazy_flag_ID < num_lazy_quantifiers; lazy_flag_ID++) {
            ...
            Integer new_finish_count = new_finish.lazy_quantifier_count.get(
                lazy_flag_ID);
